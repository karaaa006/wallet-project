...
export const AddTransaction = ({modalIsOpen, closeModal}) => {
    const [selectedCategory, setSelectedCategory] = useState(defaultValueSelected);
    ...
    const validationSchema = Yup.object().shape(
      {
        amount: Yup.number().required(() => notify('Введите сумму'))
          .min(0.01, () => notify('Сумма должна быть больше 0')),
        comment: Yup.string()
          .max(500, () => notify('Ваш комментарий слишком велик')),
        date: Yup.date().required(() => notify('Выберите дату'))
